// This is a generated file. Not intended for manual editing.
package tech.pantheon.yanginator.plugin.psi;

import java.util.List;
import org.jetbrains.annotations.*;
import com.intellij.psi.PsiElement;

public interface YangYangStmt extends YangNamedElement {

  @Nullable
  YangActionStmt getActionStmt();

  @Nullable
  YangAnydataStmt getAnydataStmt();

  @Nullable
  YangAnyxmlStmt getAnyxmlStmt();

  @Nullable
  YangArgumentStmt getArgumentStmt();

  @Nullable
  YangAugmentStmt getAugmentStmt();

  @Nullable
  YangBaseStmt getBaseStmt();

  @Nullable
  YangBelongsToStmt getBelongsToStmt();

  @Nullable
  YangBitStmt getBitStmt();

  @Nullable
  YangCaseStmt getCaseStmt();

  @Nullable
  YangChoiceStmt getChoiceStmt();

  @Nullable
  YangConfigStmt getConfigStmt();

  @Nullable
  YangContactStmt getContactStmt();

  @Nullable
  YangContainerStmt getContainerStmt();

  @Nullable
  YangDefaultStmt getDefaultStmt();

  @Nullable
  YangDescriptionStmt getDescriptionStmt();

  @Nullable
  YangDeviateAddStmt getDeviateAddStmt();

  @Nullable
  YangDeviateDeleteStmt getDeviateDeleteStmt();

  @Nullable
  YangDeviateNotSupportedStmt getDeviateNotSupportedStmt();

  @Nullable
  YangDeviateReplaceStmt getDeviateReplaceStmt();

  @Nullable
  YangDeviationStmt getDeviationStmt();

  @Nullable
  YangEnumStmt getEnumStmt();

  @Nullable
  YangErrorAppTagStmt getErrorAppTagStmt();

  @Nullable
  YangErrorMessageStmt getErrorMessageStmt();

  @Nullable
  YangExtensionStmt getExtensionStmt();

  @Nullable
  YangFeatureStmt getFeatureStmt();

  @Nullable
  YangFractionDigitsStmt getFractionDigitsStmt();

  @Nullable
  YangGroupingStmt getGroupingStmt();

  @Nullable
  YangIdentityStmt getIdentityStmt();

  @Nullable
  YangIfFeatureStmt getIfFeatureStmt();

  @Nullable
  YangImportStmt getImportStmt();

  @Nullable
  YangIncludeStmt getIncludeStmt();

  @Nullable
  YangInputStmt getInputStmt();

  @Nullable
  YangKeyStmt getKeyStmt();

  @Nullable
  YangLeafListStmt getLeafListStmt();

  @Nullable
  YangLeafStmt getLeafStmt();

  @Nullable
  YangLengthStmt getLengthStmt();

  @Nullable
  YangListStmt getListStmt();

  @Nullable
  YangMandatoryStmt getMandatoryStmt();

  @Nullable
  YangMaxElementsStmt getMaxElementsStmt();

  @Nullable
  YangMinElementsStmt getMinElementsStmt();

  @Nullable
  YangModifierStmt getModifierStmt();

  @Nullable
  YangModuleStmt getModuleStmt();

  @Nullable
  YangMustStmt getMustStmt();

  @Nullable
  YangNamespaceStmt getNamespaceStmt();

  @Nullable
  YangNotificationStmt getNotificationStmt();

  @Nullable
  YangOrderedByStmt getOrderedByStmt();

  @Nullable
  YangOrganizationStmt getOrganizationStmt();

  @Nullable
  YangOutputStmt getOutputStmt();

  @Nullable
  YangPathStmt getPathStmt();

  @Nullable
  YangPatternStmt getPatternStmt();

  @Nullable
  YangPositionStmt getPositionStmt();

  @Nullable
  YangPrefixStmt getPrefixStmt();

  @Nullable
  YangPresenceStmt getPresenceStmt();

  @Nullable
  YangRangeStmt getRangeStmt();

  @Nullable
  YangReferenceStmt getReferenceStmt();

  @Nullable
  YangRefineStmt getRefineStmt();

  @Nullable
  YangRequireInstanceStmt getRequireInstanceStmt();

  @Nullable
  YangRevisionDateStmt getRevisionDateStmt();

  @Nullable
  YangRevisionStmt getRevisionStmt();

  @Nullable
  YangRpcStmt getRpcStmt();

  @Nullable
  YangStatusStmt getStatusStmt();

  @Nullable
  YangSubmoduleStmt getSubmoduleStmt();

  @Nullable
  YangTypeStmt getTypeStmt();

  @Nullable
  YangTypedefStmt getTypedefStmt();

  @Nullable
  YangUniqueStmt getUniqueStmt();

  @Nullable
  YangUnitsStmt getUnitsStmt();

  @Nullable
  YangUsesAugmentStmt getUsesAugmentStmt();

  @Nullable
  YangUsesStmt getUsesStmt();

  @Nullable
  YangValueStmt getValueStmt();

  @Nullable
  YangWhenStmt getWhenStmt();

  @Nullable
  YangYangVersionStmt getYangVersionStmt();

  @Nullable
  YangYinElementStmt getYinElementStmt();

}
